...
import io.github.mrmarvel.schemabuild.plugin.BuildTask;
import io.github.mrmarvel.schemabuild.plugin.SchemaBuild;
import io.github.mrmarvel.schemabuild.schematic.block.NBTBlock;
import io.github.mrmarvel.schemabuild.schematic.material.BlockDataMaterial;
import io.github.mrmarvel.schemabuild.schematic.material.DirectionalBlockDataMaterial;
import io.github.mrmarvel.schemabuild.schematic.material.MultipleFacingBlockDataMaterial;
...
public class Schematic {
     ...
     */
    public Schematic(SchemaBuild plugin, File schematic) {
        this.plugin = plugin;
                ...
                for (int y = 0; y < height; ++ y) {
                    for (int z = 0; z < length; ++z) {
                        ...
                        if (material != Material.AIR) {
                            if (NBTMaterial.fromBukkit(material) == null || !Objects.requireNonNull(NBTMaterial.fromBukkit(material)).isDelayed()) {
    ...
    public void previewSchematic(Player player) {
        ...
        BuildTask task = null;
        try {
            task = new BuildTask(plugin, player);
        } catch (Exception e) {
            e.printStackTrace();
        }
        task.start();
    ...
    public Schematic unloadSchematic() {
